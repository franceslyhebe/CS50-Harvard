...
# Verificar se foi passado 3 parametros no terminal
if (len(argv) != 3):
    print("Error. Usage: databases/???.csv sequences/???.txt ")
    exit()

# Lendo o arquvio de DNA
with open(argv[2], 'r') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    for row in csv_reader:
        indexDNA = row[0]

# Lendo o arquivo Database
with open(argv[1], 'r') as csv_file:
    ...
    # Separando a primeira linha como parametro e o resto como dados
    for row in csv_reader:
        counter += 1
        if counter == 1:
            params = row
    ...
    # Passando os elementos(com exceção do primeiro) para a função getTotalSTRs
    total_STR = list(map(getTotalSTRs, params[1:]))

    # Percorrendo os dados e encontrando o nome
    for array in data:
